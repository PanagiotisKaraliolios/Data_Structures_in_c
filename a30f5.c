#include <stdio.h>
#define  MaxElements 20          //το μέγιστο πλήθος των στοιχείων του σωρού
...
FILE *f1;
f1 = fopen("transactions.txt", "r");

if(f1==NULL)
    printf("Can't open file\n");
else{
    while(TRUE){
        nscan = fscanf(f1, "%f", &AnItem.key);
        if(nscan == EOF) break;
...
void DeleteMinHeap(HeapType *Heap, HeapNode *Item)
/* Δέχεται:    Ένα σωρό Heap.
   Λειτουργία: Ανακτά και διαγράφει το μικρότερο στοιχείο του σωρού.
   Επιστρέφει: Το μικρότερο στοιχείο Item του σωρού και τον τροποποιημένο σωρό
*/
{
   int parent, child;
   HeapNode last;
